...
pacman::p_load(data.table, dplyr, tidyverse, xtable, ggplot2, glmnet, randomForest, tfdatasets, keras, car)
```
...
When we started the analysis we initially thought that the housing prices would be affected by a very few factors which may be pushing the results higher. But this is not true, instead there are a plethora of factors affecting housing prices. From our research we found a lot of significant factors affecting housing like the availability of a dishwasher, washer-dryer, number of baths, population density etc. Some factors were expected due to the monetary values associated with them but some factors like if smoking is allowed or not or pets allowed or not were very unanticipated to be significant. Hence, through this research, we were able to pinpoint the various factors that go into play for determining housing prices. 


## Issues and Limitations

The biggest issue we initially faced was with respect to the file size which turned out to be quiet massive even for R Studio. The raw data we started with was half a gigabyte big which turned out to be very massive for any for of extrapolation. Hence we had to shorten the data out choosing 20,000 data points for a particular seed to ensure reproducibility After that we also had some problems with knitting the data because of which we had to keep clearing our knitr cache to ensure we get the right data across on the PDF. Finally, the random forest part took some time running as it had to run a data heavy analysis. We had to wait this part out as we wanted to give the highest quality output. 
...
```{r normal LM, echo = FALSE, warning = TRUE, message = FALSE, results = 'hide'}
...
fit2 <- lm(price ~ pool+ dishwasher + `washer-dryer`+ ac+ parking+ state+ num_beds+ num_baths+ house_type+ sqft+ Population+ PopulationDensity + security_deposit, data = data.lm.train)
...
4.Finally, the logistic regression model includes pool, dishwasher, washer_dryer, ac, parking, state, num_baths, house_type, sqft, smoking_ind, pets_ind, Population, PopulationDensity and security_deposit. All of the variables are significant at 0.01. Among those, ac, dishwasher and Pool play relatively important roles when the prices are setting.
...
fit.logit2 <- glm(Y ~ pool+dishwasher+washer_dryer+ac+parking+state+num_beds+num_baths+house_type+sqft+
                   smoking_ind+pets_ind+Population+PopulationDensity+security_deposit, family=binomial, 
                 data = data.log.train)
summary(fit.logit2) 
...
In conclusion we find that various factors affect housing prices, beyond the simple square feet and location. The various micro-factors like availability of a pool, washer-dryer, dishwasher etc. play an important role as well and shouldn't be forgotten when considering if a housing rental price. Our data is most applicable to Apartment and Condo/Multiplex form of apartments, though with repeated testing our results can be extended to House, Townhome, Single Room and Sublease or Student contract form of rentals as well. There was a also a huge variability noticed in the state where the houses are located with states like New York being more expensive than mid western states or southern states. This can also be explained by the per capita incomes at these places which can drastically differ. We also ran a random forest mdeol to predict the values ourselves with a huge accuracy. We find that random forest is the best model to predict the housing prices, combined with simple logistic regression.
